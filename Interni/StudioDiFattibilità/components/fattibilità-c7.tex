...
Il capitolato richiede lo sviluppo di un algoritmo e di un front-end per sincronizzare file tra server e client appoggiandosi sul sevizio \glo{Zextras Drive}.
...
\begin{itemize}
\item \textbf{Sviluppo di un algoritmo di sincronizzazione \glo{solido} ed efficiente}: deve garantire la sincronizzazione ottimale tra cloud e desktop utilizzando il quantitativo minimale di risorse;
...
\item \textbf{Integrazione con \glo{Zextras Drive}}: sia l'algoritmo che l'interfaccia devono essere integrati con il servizio di gestione file \glo{Zextras Drive}.
\end{itemize}
...
\begin{itemize}
    ...
    Semplifica il processo di integrazione con le \glo{API} di \glo{Zextras Drive};
    \end{itemize}
\newpage
\item \textbf{Tecnologie necessarie}:
    \begin{itemize}
    \item \textbf{\glo{Zimbra}}: \glo{sistema} collaborativo per la gestione della posta elettronica;
    \item \textbf{\glo{Zextras Drive}}: servizio proprietario di gestione file in cloud con funzionalità di:
        \begin{itemize}
        \item Versionamento;
...
\item \textbf{Altre tecnologie}:
    \begin{itemize}
    \item \textbf{\glo{GraphQL}}: linguaggio di query per \glo{API} utilizzato dalle \glo{API} di \glo{Zextras Drive};
    \item \textbf{Apollo per \glo{GraphQL}}: implementazione di \glo{GraphQL} utilizzata dalle \glo{API} di \glo{Zextras Drive}.
    \end{itemize}
\end{itemize}
...
\item il \glo{team} ha già familiarità con il \glo{framework Qt} e Python.
\end{itemize}

\subsection{Criticità e fattori di rischio}
\begin{itemize}
\item mancanza di documentazione dettagliata pubblicamente accessibile sulle \glo{API} del prodotto \glo{Zextras Drive};
