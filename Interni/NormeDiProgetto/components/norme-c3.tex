...
Le attese del \glo{team} per questo processo sono individuare una struttura comune per tutti i prodotti del processo e raccogliere tutte le norme comuni per la stesura dei documenti.
...
\begin{itemize}
	...
	\begin{itemize}
		\item \textbf{NomeDelDocumento}: il nome ufficiale del documento, senza spazi tra le parole e con solamente le iniziali di ogni parola in maiuscolo (convenzione \glo{PascalCase});
...
\setlength{\freewidth}{\dimexpr\textwidth-0\tabcolsep}
	...
	\begin{longtable}{L{.30\freewidth} L{.60\freewidth}}
		...
		\toprule
		\endhead
		\textbf{\ignore{GitHub} Desktop} & Client ufficiale di \glo{GitHub} utilizzato per la gestione delle \glo{repository} \glo{Git}. \\
...
\subsubsection{Descrizione}
Per l'accertamento della qualità è dedicato il documento \PdQ{}, descrivente le \glo{metriche} e le modalità usate per valutare il livello di qualità di prodotti e processi. Il documento fissa i \glo{requisiti} qualitativi e le \glo{metriche} relative ai processi impiegati e ai prodotti sviluppati. Quest'ultimo viene costantemente aggiornato per tenere traccia di eventuali cambiamenti ed è strutturato in modo tale da distinguere la parte riguardante la documentazione e quella riguardante il \glo{software}.
...
\begin{itemize}
	\item conseguimento della qualità nel prodotto, secondo le richieste del \glo{proponente}. \\
...
\setlength{\freewidth}{\dimexpr\textwidth-0\tabcolsep}
	...
	\begin{longtable}{L{.15\freewidth} L{0.35\freewidth} L{0.5\freewidth}}
		...
		\multirow{2}{.15\freewidth}{\textbf{Analisi statica}}
		 & \multirow{2}{0.35\freewidth}{L'analisi statica permette di effettuare controlli su documenti e codice, verificando così che la corretta applicazione della correttezza.}&\textbf{Walkthrough:} tecnica di \glo{verifica} consistente nella lettura da parte del \glo{team} dell'intero documento o codice in cerca di anomalie. Verrà applicata principalmente nella fase iniziale del progetto, quando il \glo{team} ancora non conosce i possibili errori che si possono incontrare. Questa tecnica risulta molto onerosa in termini di efficienza ed efficacia. \\
		 \cline{3-3} 
...
\subsubsection{Strumenti}
Gli strumenti che il gruppo ha scelto di utilizzare per la \glo{verifica} automatica del codice sono:
...
\setlength{\freewidth}{\dimexpr\textwidth-0\tabcolsep}
	...
	\begin{longtable}{L{.15\freewidth} L{.75\freewidth}}
		...
		\textbf{Codecov} & Servizio online per l'analisi del codice. Effettua l'analisi dinamica del codice ad ogni push. Restituisce grafici e informazioni inerenti la quantità di codice coperto da \glo{test}. \\
		\textbf{unittest} & Framework di \glo{test} \textit{Python}, utilizzato per effettuare \glo{test} dinamici sul codice, in particolare \glo{test di \ignore{unità}} e di integrazione. \\
		\bottomrule
		\hiderowcolors
...
\setlength{\freewidth}{\dimexpr\textwidth-0\tabcolsep}
	...
	\begin{longtable}{L{.10\freewidth} L{.5\freewidth} L{.280\freewidth}}
		...
		\endhead
				...
				\textbf{MVR04} & Rappresenta la percentuale dei \glo{test} superati sul totale dei \glo{test} eseguiti. &  \small{V04=$\frac{n^\circ\  \ignore{test}\ superati*100}{n^\circ\  \ignore{test}\ eseguiti}$} \\
				\textbf{MVR05} & Failed \glo{Test} Cases Percentage, rappresenta la percentuale dei \glo{test} falliti sul totale dei \glo{test} eseguiti. & \small{V05=$\frac{n^\circ\  \ignore{test}\ non\ superati*100}{n^\circ\  \ignore{test}\ eseguiti}$}\\
		\bottomrule
		\hiderowcolors
...
Lo scopo del processo di \glo{validazione} è determinare se il prodotto è in grado di soddisfare il compito per il quale è stato creato e se rispetta i \glo{requisiti} e le aspettative del committente. Dovranno essere rispettate le seguenti aspettative:

\begin{itemize}
	\item gli oggetti da validare sono stati identificati;
	\item avere ed applicare una strategia con procedure di \glo{validazione} utili per rendere le procedure di \glo{verifica} riutilizzabili;
	\item valutazione dei risultati ottenuti rispetto a quelli attesi. 
\end{itemize}

\newpage{}
\subsubsection{Attività di validazione}
La \glo{validazione} del codice si compone di due attività:
\setlength{\freewidth}{\dimexpr\textwidth-0\tabcolsep}
	\renewcommand{\arraystretch}{1.5}
	\setlength{\aboverulesep}{0pt}
	\setlength{\belowrulesep}{0pt}
	...
	\begin{longtable}{L{.20\freewidth} L{.70\freewidth}}
		...
		\toprule
		\endhead
		\textbf{\glo{Test} di \glo{validazione}} & Insieme delle attività svolte internamente al gruppo.\\
		\textbf{Collaudo} & Attività di controllo che si svolge con la presenza del committente durante la \emph{Revisione di Accettazione}. Il gruppo deve quindi dimostrare di aver implementato tutti i \glo{requisiti}, in questo senso, per una corretta e facile \glo{verifica}, è fondamentale il \glo{tracciamento} dei \glo{test} e dei \glo{requisiti} che validano.\\
		\bottomrule
		\hiderowcolors
		\caption{Attività del processo di validazione del codice}
	\end{longtable}

Il processo di \glo{Validazione} è composto dalle seguenti attività:
\begin{itemize}
	\item pianificazione e \glo{tracciamento} dei \glo{test} da eseguire sul codice prodotto;
	\item esecuzione dei \glo{test};
	\item aggiornamento del \PdQ{};
	\item controllare che il \glo{sistema} soddisfi i \glo{requisiti}.
\end{itemize}

\subsubsection{Responsabilità dei test}
I soggetti coinvolti nell'ambito dei \glo{test} di \glo{validazione} sono:
...
\setlength{\freewidth}{\dimexpr\textwidth-0\tabcolsep}
	...
	\begin{longtable}{L{.20\freewidth} L{.70\freewidth}}
		...
		\toprule
		\endhead
		\textbf{\progs{}} & Si occupano della pianificazione e implementazione dei \glo{test}.\\
		\textbf{\vers{}} & Eseguono i \glo{test} sul \glo{software} e aggiornano il \PdQ{} con i risultati ottenuti.\\
		\textbf{\RdP{}} & Osserva i risultati ottenuti per mezzo del \PdQ e decide se accettare il risultato ottenuto, nel caso in cui i risultati dei \glo{test} risultano sufficienti, oppure ripetere. \\
		\bottomrule
		\hiderowcolors
		\caption{Attività del processo di validazione del codice}
	\end{longtable}
