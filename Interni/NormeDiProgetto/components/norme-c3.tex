...
\begin{itemize}
\item \textbf{test di validazione}: insieme delle attività svolte internamente al gruppo;
\item \textbf{collaudo}: attività di controllo che si svolge con la presenza del committente durante la \emph{Revisione di Accettazione}. Il gruppo deve quindi dimostare di aver implementato tutti i requisiti, in questo senso, per una corretta e facile verifica, è fondamentale il \Glo{tracciamento} dei test e dei requisiti che validano.
\end{itemize}

Il processo di Validazione è composto dalle seguenti attività:
\begin{itemize}
\item \textbf{pianificazione e tracciamento} dei test da eseguire sul codice prodotto;
\item \textbf{esecuzione dei test};
\item \textbf{aggiornamento del \emph{Piano di qualifica}};
\item \textbf{controllo dei risultati}: dove viene verificato se il sistema soddisfi i requisiti.
\end{itemize}

\subsubsection{Responsabilità dei test}
I soggetti coinvolti nellàambito dei test di validazione sono:
\begin{itemize}
\item \textbf{Progettisti}: si occupano della pianificazione e implementazione dei test;
\item \textbf{Verificatori}: eseguono i test sul software e aggiornano il \PdQ con i risultati ottenuti
\item \textbf{Responsabile di progetto}: osserva i risultati ottenuti per mezzo del \PdQ e decide se accettare il risultato ottenuto, nel caso in cui i risultati dei test risultano sufficienti, oppure ripetere.
\end{itemize}
