...
Per ottenere un riscontro efficace sul lavoro svolto, il gruppo \Gruppo{} propone di instaurare e mantenere un dialogo continuo e un costante rapporto collaborativo con il \Glo{proponente} \proponente{}.
...
\subsection{Sviluppo}
\paragraph*{Descrizione}
Il processo di svilupop si propone di descrivere i compiti e le attività da svolgere in modo da realizzare al meglio il prodotto richiesto dal \glo{proponente}. Le attività coinvolte riguardano l'Analisi dei \ignore{Requisiti}, la progettazione e la codifica.
\paragraph*{Aspettative}
Con il processo in questione ci si aspetta di stabilire gli obiettivi di sviluppo, i vincoli tecnologici e di design, e la realizzazione di un prodotto finale che superi i test progettati, e che soddisfi appieno i requisiti e le richieste del proponente.
...
\end{itemize}
\paragraph*{Aspettative}
Dall'attivita di Analisi il gruppo si aspetta la produzione di documentazione formale, che racchiuda tutti i requisiti trovati per mezzo delle diverse fonti sopra citate.
\paragraph*{Struttura} 
...
\setlength{\freewidth}{\dimexpr\textwidth-0\tabcolsep}
	...
	\end{longtable}
\paragraph*{Aspettative}
Con l'attività di Progettazione il gruppo si aspetta di sviluppare un'architettura che rispetti i vincoli imposti dal \Glo{proponente}, e che implenti in modo adeguato i design pattern che il gruppo sceglie.
...
\subsubsection{Metriche}
{
	...
	\begin{longtable}{L{.1\freewidth} L{.15\freewidth} L{.4\freewidth} L{.2\freewidth}}
		...
		\textbf{MCD06} & Lunghezza della riga di codice & Per facilitare la lettura e la riduzione della complessità di una singola riga di codice non sarà possibile avere righe di codice più lunghe di quanto stabilito dalla metrica & lunghezza riga <= 100 caratteri\\
		...
		\textbf{MCD08} & Fan-in & Indica quante componenti utilizzano un modulo. & Fan-in $\geq 0$ \\
		\textbf{MCD09} & Fan-out & Indica quante componenti vengono utilizzate dalla componente in esame. & Fan-out $\leq 6 $ \\
		\textbf{MCD10} & Coupling Between Object. & Misura il numero di classi che sono accoppiate a una particolare classe. Un accoppiamento eccessivo rende il sistema più difficile da gestire e riutilizzare quindi più è alto il grado di coupling della classe e più il sistema è difficile da mantenere. & CBO = numero delle dipendenze \\
		\textbf{MCD11} & Numero di parametri per metodo. & Un elevato numero di parametri presenti all'interno dello stesso metodo indica il bisogno di ridurre le funzionalità associate a tale metodo. Più è elevato questo valore e più aumenta la possibilità di commettere errori progettuali. & NPM= numero dei paramentri \\
		\textbf{MCD12} & Livello di annidamento. & Indica il livello di annidamento di un metodo. Un alto livello di annidamento porta a un codice di difficile manutenzione, aumentandone la complessità, in questo caso conviene dividere il metodo in più metodi distinti. & LA = livello di annidamento \\
		\bottomrule
		\hiderowcolors
