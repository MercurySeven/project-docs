...
\begin{itemize}
	\item Creare una variabile "\textit{\_\_create\_key}" ed assegnarle un nuovo object: \texttt{\_\_create\_key = object()};
	...
	\item All'interno del costruttore "\textit{\_\_init\_\_(self, key)}" avere la seguente istruzione \texttt{assert(key == \_\_create\_key)}. In questo modo il costruttore potrà essere chiamato solamente all'interno della classe stessa e sarà get\_istance a fare da unico punto d'accesso pubblico esterno.
\end{itemize}
Se si volesse avere anche un Singleton thread-safe si deve implementare un sistema di lock (\url{https://docs.python.org/3/library/threading.html}) all'interno del metodo get\_istance evitando che due metodi si contendano il suo utilizzo.
