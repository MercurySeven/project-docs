...
All'avvio verrà mostrata una schermata che servirà ad autenticarsi, bisognerà inserire nome utente e password del proprio account \gloman{Zextras Drive}, in caso di successo dell'operazione si avvierà il software, altrimenti verrà visualizzato un messaggio di errore e chiesto di reinserire le credenziali.
\begin{figure}[H]
    \centering
...
\begin{enumerate}
	...
	\begin{itemize}
		\item \textbf{Client}: indica la politica nel quale i file presenti in locale hanno sempre la priorità sui file nel \gloman{server}, vengono scaricati solamente file non presenti localmente;
		\item \textbf{Manuale}: indica la politica nella quale se è presente un conflitto tra la versione del file locale con quella del \gloman{server} allora viene richiesta l'interazione dell'utente.
	\end{itemize}
