...
L'obbiettivo di questa fase è di assicurarsi che il codice creato soddisfi i \glo{requisiti} qualitativi imposti dalle \glo{metriche}. Viene quindi scritto il \glo{test} assicurandosi che esso non sia soddisfatto dal codice presente al momento della creazione del \glo{test}. Una volta sviluppato il \glo{test} si procede a scrivere la quantità minima di codice necessaria per soddisfare il \glo{test}, si passa poi alla fase di refactoring.
...
Si compongono essenzialmente dei \glo{test} di \glo{sistema} però eseguiti in sede aziendale.
