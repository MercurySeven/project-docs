...
Il modello di sviluppo \glo{agile} permette di progredire tramite cicli iterativi, detti sprint.
...
\begin{enumerate}
    ...
    \item \textbf{La collaborazione col cliente più che la negoziazione dei contratti}: un contratto inflessibile può essere una potenziale perdita di valore per per tutte le parti coinvolte. Va quindi incentivata l’interazione con gli \glo{stakeholder} durante lo sviluppo del progetto;
...
Il lavoro viene quindi suddiviso in piccole funzionalità chiamate \textbf{\glo{User Story}} le quali poi vengono selezionate per essere portate a termine negli \textbf{sprint}, venendo, di fatto, sviluppate indipendentemente in una sequenza continua dall’analisi all’integrazione. \newline
...
e la grande attenzione posta alla comunicazione sia interna che esterna con tutti gli \glo{stakeholder} facendo sì che le richieste del \proponente{} vengano soddisfatte a pieno.
...
\begin{itemize}
    ...
    \item \textbf{Coinvolgimento degli \glo{stakeholder}}: Per l’attuazione del modello di sviluppo \glo{agile} è fondamentale avere un feedback costante da parte degli \glo{stakeholder}. Per questo motivo, ogni scelta implementativa e realizzativa presa dal \glo{team}, dovrà essere confermata da questi ultimi;
    ...
    \item \textbf{Discussione feedback}: Durante l’ultimo giorno di sprint, il \glo{team} dovrà quantificare e valutare obiettivamente il lavoro svolto in quel periodo di tempo, oltre a dover proporre migliorie o altre caratteristiche da implementare.
\end{itemize}